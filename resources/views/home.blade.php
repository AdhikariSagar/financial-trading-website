<x-main-layout>
    ...
    <div class="mt-8 w-full flex flex-row space-y-4 md:flex-row md:space-x-4 md:space-y-0">
        ...
        <div class="w-full md:w-[65%]">
            {{-- news--details --}}
            <!-- News section similar to the provided image -->
            ...
            <div class="bg-slate-200 p-2 border border-gray-300 rounded-md space-y-2">
                ...
                <div class="w-full flex flex-row justify-between items-center p-2 border-b border-gray-400">
                    <div class="flex flex-row items-center space-x-4">
                        @php
                            // Construct the image source URL based on the item name
                            $imageName = strtolower(str_replace(' ', '-', $extractedNames[$index]));
                            $imageSrc = asset("img/$imageName.png");
                        @endphp
                        <img class="h-12 w-12 rounded-full" src="{{ $imageSrc }}" alt="{{ $extractedNames[$index] }}">
                        <div>
                            <a href="/instrument-details/{{$getInstrumentType($item->type)}}/{{ $item->symbol }}" class="font-semibold cursor-pointer hover:text-blue-400 max-w-[100px] overflow-hidden"><span>{{ $extractedNames[$index] }}</span></a>
                            <p class="text-sm text-gray-400 leading-none">{{$item->isin}}</p>
                        </div>
                    </div>
                    <div class="flex flex-col text-sm text-gray-400 content-center mt-1 space-x-1">
                        
                        <a href="/instrument-details/{{$getInstrumentType($item->type)}}/{{ $item->symbol }}" class="font-semibold cursor-pointer {{$latestCandleData[$index]>0 ? 'text-green-500' : 'text-red-600'}} hover:text-blue-400">{{$latestCandleData[$index]>0 ? '+' :''}}{{$latestCandleData[$index]}}%</a>
                        
                        <span class="text-xs px-3 leading-none pb-1 pt-1 bg-slate-300 rounded-full w-fit">{{ $getInstrumentType($item->type) }}</span>
